...
describe('generate_audio', () => {
  ...
  it('should generate audio file and return correct hash', async () => {
    ...
    expect(result.audioPath).toMatch(new RegExp(`^${tempDir.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}/[a-f0-9]{64}\\.mp3$`));
  ...
  it('should handle different voice options', async () => {
    ...
    const result = await generate_audio(testText, { voice: 'nova', model: 'tts-1-hd', audioDir: tempDir });
